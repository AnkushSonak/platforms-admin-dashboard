...
import type { AnswerKey } from '@/app/helper/interfaces/AnswerKey';
...
import { ANSWER_KEYS_API } from '@/app/envConfig';
...
export default function AdminAnswerKeysPage() {
  ...
  const fetchAnswerKeys = useCallback(async () => {
    ...
    setError(null);
    try {
      // const result = await getAnswerKeys(searchQuery, currentPage, ANSWERKEYS_PER_PAGE);
      const result = await getPaginatedEntity<AnswerKey>("type=answer-keys&page=1", ANSWER_KEYS_API,  { entityName: "answerKeys" })
      setAnswerKeys(result.data);
  ...
  const handleDeleteAnswerKey = async (answerKeyId: string) => {
    setDeletingAnswerKeyId(answerKeyId);
    try {
      const response = await fetch(`${API_BASE_URL}/answerKey/${answerKeyId}`, {
        method: 'DELETE',
        headers: {
      ...
      if (response.ok) {
        ...
        fetchAnswerKeys(); // Re-fetch to update the list and pagination
      } else if (response.status === 401 || response.status === 403) {
        setError("Unauthorized to delete. Please log in as an admin.");
        // checkAuth();
      } else {
        const errorData = await response.json();
        setError(errorData.message || 'Failed to delete answer key.');
      }
    } catch (err: any) {
          ...
          {loading ? (
            ...
            <div className="overflow-x-auto">
              <Table>
                ...
                <TableBody>
                  {answerKeys.length === 0 ? (
                    <TableRow><TableCell colSpan={6} className="text-center">No answer keys found.</TableCell></TableRow>
                  ) : answerKeys.map(card => (
                    <TableRow key={card.id}>
                      <TableCell>{card.answerKeyTitle}</TableCell>
                      <TableCell>{card.answerKeyExamName}</TableCell>
                      <TableCell>{card.answerKeyStatus}</TableCell>
                      <TableCell>{card.answerKeyReleaseDate ? format(new Date(card.answerKeyReleaseDate), 'yyyy-MM-dd') : '-'}</TableCell>
                      <TableCell className="flex gap-2">
                        ...
                        <AlertDialog>
                          <AlertDialogTrigger asChild>
                            <Button variant="destructive" size="icon" className="h-8 w-8" disabled={deletingAnswerKeyId === card.id}>
                              {deletingAnswerKeyId === card.id ? (
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>
                                This action cannot be undone. This will permanently delete the Answer Key posting &quot;{card.answerKeyTitle}&quot;.
                              </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
                              <AlertDialogCancel>Cancel</AlertDialogCancel>
                              <AlertDialogAction onClick={() => handleDeleteAnswerKey(card.id)} className="bg-red-600 hover:bg-red-700">
                                Delete
                              </AlertDialogAction>
                            </AlertDialogFooter>
                          </AlertDialogContent>
